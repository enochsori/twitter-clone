...
import { TweetInterFace } from '../components/time-line';
...
const Profile = () => {
  ...
  const onChangeAvatar = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length === 1) {
      ...
      // set location to save the new user image
      const locationRef = ref(storage, `avatar/${user?.uid}`);
      // upload the user image and get the result to use its url
      const result = await uploadBytes(locationRef, file);
      // get the user image url in storage
